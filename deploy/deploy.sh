...
if [ -z "$2" ]
    then
        echo "Domain was not supplied, defaulting to developmentnow.net"
        domain=developmentnow.net
    else
        domain=$2
fi
...
if [ -z "$3" ]
    then
        echo "Email was not supplied, defaulting to ben.kauffman@developmentnow.com"
...
# Build image for
docker build --no-cache=true -t $name --build-arg env=$name --file Dockerfile .
...
docker run -d \
  ...
  --restart always \
  -v /home/ec2-user/logs/$name:/usr/dev/project11-web-api/logs \
  $name
